...
test('doc worker generates docs only for undocumented symbols when nothing is stale', async () => {
  ...
  // Existing documentation exists for FN1 (fresh); nothing is stale.
  const existing = docStore.upsertBlock({
    tenantId,
    repoId,
    docFile: 'contracts/function/src/a.ts/src-a-ts-f1.md',
    blockAnchor: '## src/a.ts::f1',
    blockType: 'function',
    content: '## src/a.ts::f1\n',
    status: 'fresh'
  });
  docStore.setEvidence({ tenantId, repoId, blockId: existing.id, evidence: [{ symbolUid: 'FN1' }] });
  ...
  assert.ok(paths[0].includes('src-b-ts-f2'));
