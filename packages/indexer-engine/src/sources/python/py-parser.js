...
export function* parsePythonFile({ filePath, lines, sha, containerUid, exportedByFile, packageToUid }) {
  ...
  for (const imp of parsePyImports(lines)) {
    ...
    if (!pkg) continue;
    // Python package ecosystem mapping is future; for now, record as observed dependency without linking a package node.
    if (packageToUid?.has?.(`pypi:${pkg}`)) {
      const packageKey = `pypi:${pkg}`;
      const pkgUid = packageToUid.get(packageKey);
      yield { type: 'observed_dependency', data: { file_path: filePath, sha, package_key: packageKey, evidence: { import_module: imp.module, line: imp.line } } };
      yield {
        type: 'edge',
        data: {
          source_symbol_uid: sourceUid,
