...
function parseGemfile(text) {
  ...
  for (const line0 of lines) {
    ...
    if (!m) continue;
    deps.push({ name: m[1], version: m[2] ?? '*', scope: 'prod' });
  }
  return { deps };
}

export function parseGemfileManifest({ absManifestPath, filePath, sha, packageToUid }) {
  const records = [];
  const manifestNode = makeManifestNode({ filePath, sha });
  records.push({ type: 'node', data: manifestNode });

  const parsed = parseGemfile(fs.readFileSync(absManifestPath, 'utf8'));
  records.push({
    type: 'dependency_manifest',
    data: { file_path: filePath, sha, manifest_type: 'Gemfile', manifest_key: `${filePath}::${sha}`, parsed: { deps_count: parsed.deps.length } }
  });
