...
function parseYarnLock(text, { max = 1200 } = {}) {
  const deps = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentName) {
      ...
      if (vm) {
        deps.push({ name: currentName, version: vm[1] });
        currentName = null;
...
export function parseYarnLockManifest({ absManifestPath, filePath, sha, packageToUid }) {
  const records = [];
  const manifestNode = makeManifestNode({ filePath, sha });
  records.push({ type: 'node', data: manifestNode });

  const deps = parseYarnLock(fs.readFileSync(absManifestPath, 'utf8'));
  records.push({
    type: 'dependency_manifest',
    data: { file_path: filePath, sha, manifest_type: 'yarn.lock', manifest_key: `${filePath}::${sha}`, parsed: { deps_count: deps.length } }
  });
