...
function parseGoSum(text, { max = 800 } = {}) {
  const deps = new Map(); // module -> version
  const lines = String(text ?? '').split('\n');
  for (const line0 of lines) {
    if (deps.size >= max) break;
    ...
    const mod = m[1];
    const ver = m[2];
    if (!deps.has(mod)) deps.set(mod, ver);
  }
  return { deps: Array.from(deps.entries()).map(([name, version]) => ({ name, version })) };
}

export function parseGoSumManifest({ absManifestPath, filePath, sha, packageToUid }) {
  const records = [];
  const manifestNode = makeManifestNode({ filePath, sha });
  records.push({ type: 'node', data: manifestNode });

  const parsed = parseGoSum(fs.readFileSync(absManifestPath, 'utf8'));
  records.push({
    type: 'dependency_manifest',
    data: { file_path: filePath, sha, manifest_type: 'go.sum', manifest_key: `${filePath}::${sha}`, parsed: { deps_count: parsed.deps.length } }
  });
