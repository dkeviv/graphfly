...
function parseDecls(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const cm = line.match(/^\s*(data\s+)?class\s+([A-Za-z_][A-Za-z0-9_]*)\b/);
    if (cm) decls.push({ kind: 'class', name: cm[2], line: i + 1 });
    const fm = line.match(/^\s*fun\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/);
...
function parseCalls(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = String(lines[i] ?? '');
    if (line.includes('fun ')) continue;
    const m = line.match(/\b([A-Za-z_][A-Za-z0-9_]*)\s*\(/);
    if (!m) continue;
    const name = m[1];
    if (name === 'if' || name === 'for' || name === 'while' || name === 'when') continue;
...
function makeSymbolNode({ kind, name, filePath, line, sha, containerUid }) {
  ...
  const signature = kind === 'class' ? `class ${name}` : `fun ${name}()`;
  const signatureHash = computeSignatureHash({ signature });
  const symbolUid = makeSymbolUid({ language: 'kotlin', qualifiedName, signatureHash });
...
export function* parseKotlinFile({ filePath, lines, sha, containerUid, exportedByFile, packageToUid }) {
  ...
  for (const imp of parseImports(lines)) {
    const root = imp.path.split('.')[0] ?? null;
    if (!root) continue;
    const packageKey = `maven:${root}`;
    ...
    if (!pkgUid) continue;
    yield { type: 'observed_dependency', data: { source_symbol_uid: sourceUid, file_path: filePath, sha, package_key: packageKey, evidence: { import_path: imp.path, line: imp.line } } };
